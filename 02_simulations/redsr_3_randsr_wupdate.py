    ...
    while True:
        ...
        if (current_state + 1) == start_state:
            ...
            ## free-choice trials of other index than 0-3
                ...
                next_values_arr = np.asarray(next_values, dtype=float)
                if len(next_values) == 1:
                    next_move = 0
                else:
                    # Guard against NaNs/Infs corrupting softmax probabilities
                    if not np.all(np.isfinite(next_values_arr)):
                        # Skip this participant/run: the simulation has entered an invalid numerical regime
                        raise FloatingPointError("Non-finite values in next_values during action selection")
                    else:
...
def test_deterministic(model_parameters):
    ...
    the higher-value choice (it's deterministic).
