...
'''
Hybrid agent: mixture of MF and redsr_4_randsr_wupdate.
...
def run_trial_sr_mf(phase, trial_index, gamma, alpha_td, alpha_m, beta, mix_w, end_state, start_state, forced_choice_switch, forced_choice_trial_order, rewards, transitions, num_pairs, v_state_non_mf, feat, weight, v_state_mf):
    ...
    while True:
        ...
        ###### Middle states ######
            ...
            ###### No update of the successor matrix row correpsonding to last state (immediately rigid after MB learning) ######
...
def learning(gamma, alpha_td, alpha_m, beta, mix_w, end_state, rewards, transitions, model_parameters, forced_choice_switch):
    ...
    new_params = [num_pairs, v_state_non_mf, feat, reduced_feat, weight, reduced_weight, v_state_mf]
...
def relearning(condition, gamma, alpha_td, alpha_m, beta, mix_w, end_state, rewards, transitions, model_parameters):
    ...
    num_pairs, v_state, feat, reduced_feat, weight, reduced_weight, v_state_mf = model_parameters
    phase = "relearning"
    ...
    for trial_index, start_state in enumerate(start_states):
        v_state, reduced_feat, reduced_weight, v_state_mf, transition_log_lines = run_trial_sr_mf(
            phase,
            trial_index,
            gamma,
            alpha_td,
    ...
    new_params = [num_pairs, v_state, feat, reduced_feat, weight, reduced_weight, v_state_mf]
...
def test_deterministic(model_parameters, mix_w):
    ...
    num_pairs, v_state_non_mf, feat, reduced_feat, weight, reduced_weight, v_state_mf = model_parameters
